...
export default function WorkspacesSection() {
  ...
  const removeQuery = async (path: string) => {
    const name = (path.split('/').pop() ?? path).replace(/\.sql$/i, '')
    if (!(await confirm({ title: 'Delete query', message: `Delete "${name}"? This can't be undone.`, confirmLabel: 'Delete', danger: true }))) return
    closeQueryByPath(path); run(deleteQuery(path))
  }
  const removeWorkspace = async (name: string) => {
    if (!(await confirm({ title: 'Delete workspace', message: `Delete workspace "${name}" and all its queries? This can't be undone.`, confirmLabel: 'Delete', danger: true }))) return
